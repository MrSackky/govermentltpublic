    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await form.parse(req, async (err, fields, files) => {
          ...
          console.log("files")
          console.log(files)
          const upload = await multer({ dest: `public/uploads/c-${fields.id}/activities/` })
          const _files = []
          for (var i in files) {
            // console.log(files[i].path)
            const currentTimeInSeconds = Math.floor(Date.now() / 1000); //unix timestamp in seconds
            fs.renameSync(files[i].path, `public/uploads/c-${fields.id}/activities/${currentTimeInSeconds + '-' + files[i].name}`);
            var data = {
